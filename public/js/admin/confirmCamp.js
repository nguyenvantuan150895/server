$(function () {
    ...
    $("#save").click(() => {
        ...
        if (username.length == 0) alert("Username must be filled out!");
            ...
            $.post("/admin/manager/campaign/confirm", ob_data).done(function (customer) {
                if (customer.state == "ok") {
                    alert("Success!");
                    let path = '/admin/manager/campaign/'+customer.last_page;
...
// else if (groupArr.length == 0) alert("Must have at least one face group ");
        // else if (groupArr.length > 0) {
        //     for (let j = 0; j < groupArr.length; j++) {
        //         if (groupArr[j].trim().length == 0) alert("Name group must be filled out! ");
        //     }
        // }
        // // else if (fbArr.length == 0) alert("Must have at least one face group ");
        // else if (fbArr.length > 0) {
        //     for (let j = 0; j < fbArr.length; j++) {
        //         if (fbArr[j].trim().length == 0) alert("Url for group facebook must be filled out! ");
        //     }
        // }