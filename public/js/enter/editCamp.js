$(function () {
    let ob_camp = $("#idEditCamp").attr("customer");
    ob_camp = JSON.parse(ob_camp);
    let idCamp = ob_camp.idCamp;
    ...
    $("#save").click(() => {
        ...
        let ob_data = {
            name: name, oldUrl: oldUrl, email: email,
        ...
        if (name.length == 0) alert("Name Campaign must be filled out!");
            ...
            $.post("/enterprise/editCamp", ob_data).done(function (customer) {
                if (customer.state == "ok") {
                    alert("Success!");
                    let path = "/enterprise/detailCamp/"+idCamp;
                    window.location = path;
                } else if (customer.state == "fail") {
                    if (customer.existNameCamp == true) alert("Name Campaign already exists!");
